...
from rag.setup import full_text_db_setup, vector_db_setup
...
def load_and_embed_csv():
    ...
    for i, row in data.iterrows():
        ...
        result = vector_db_collection.query.fetch_objects(
            filters=wvc.query.Filter.by_property("course_code").equal(
                int(row["course_code"])
            ),
            limit=1,
        )
        ...
        date_text = f"{row['day_and_period']}に開講される授業である。"
        ...
        full_texts.append(
            ClassInfo(text=full_text, course_code=int(row["course_code"]))
        )
